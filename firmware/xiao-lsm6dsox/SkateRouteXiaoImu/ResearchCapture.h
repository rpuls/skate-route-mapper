...
// Pointed at by the stream views so a reader can see a capture is still
// filling. `state` cannot serve: COMPLETE is non-zero but not active.
uint8_t recordingFlag = 0;
// Set from loop(); START needs it and no longer receives it as an argument.
...
void finish(uint8_t failure = 0) {
  ...
  finishWindow();
  error = failure; state = failure ? FAILED : COMPLETE; recordingFlag = 0;
...
void beginCapture(uint32_t seconds, uint32_t requestedRate) {
  ...
  if (!raw || target > capacity) { state = FAILED; error = MEMORY_ERROR; recordingFlag = 0; return; }
  ...
  startedMs = millis(); startedUs = micros(); state = RECORDING; recordingFlag = 1;
}
void captureTick() {
  if (!recording()) return;
  uint8_t status[2];
...
/**
 * Ops 1-4, which predate the stream layer and keep their wire meaning.
 *
 * The transport owns the characteristics, the command queue and the framing
 * now. What is left here is the only part that knows what a capture is, which
 * is the right amount for this file to know.
 */
void handleCommand(uint8_t op, uint32_t requestedId, uint32_t arg, stream::Reply &reply) {
  uint8_t *p = reply.payload;
  if (op == START) {
    ...
    if (recording() || !imuAvailable || (seconds != 10 && seconds != 30 && seconds != 60) ||
        (selectedRate != DEFAULT_RATE && selectedRate != HIGH_RATE) ||
        seconds * selectedRate > capacity) reply.flags = 1;
  ...
  if (op == STATUS || op == START) {
    ...
    // Page sizes stay at 64/16 although the transport now allows more: the
    // client estimates remaining pages from these, and captures already
    // uploaded were timed against them.
    uint32_t total = op == RAW ? count : windows;
    uint16_t stride = op == RAW ? 6 : 24, maximum = op == RAW ? 64 : 16;
    if (recording() || !captureId || requestedId != captureId || arg >= total) reply.flags = 2;
    else {
      reply.records = total - arg > maximum ? maximum : total - arg;
      reply.bytes = reply.records * stride;
      memcpy(p, (op == RAW ? raw : summaries) + arg * stride, reply.bytes);
    }
  } else reply.flags = 3;
  // The capture the data actually belongs to, not the one asked for. A client
  // comparing the two is how a page from a superseded capture is caught.
  reply.streamId = captureId;
  reply.offset = arg;
}
...
void setup() {
  ...
  // The same bytes, reachable through the generic read path as well. Nothing
  // is copied and the capture keeps its own lifecycle; this only means a
  // client that speaks streams does not need to special-case research.
  if (raw) {
    stream::declareView(stream::ID_RESEARCH_RAW, stream::KIND_RESEARCH_RAW, 6,
                        raw, capacity, &count, &recordingFlag);
  }
  stream::declareView(stream::ID_RESEARCH_SUMMARIES, stream::KIND_RESEARCH_SUMMARY, 24,
                      summaries, MAX_WINDOWS, &windows, &recordingFlag);
}
} // namespace research
